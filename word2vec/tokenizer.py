...
def get_tokenizer(
    ...
    try:
        tokenizer = Tokenizer.from_file(f'tokenizers/{subset}-vocab_{min_frequency}_{size_str_mil}.json')
    ...
    if data is None:
        ...
        data = [add_spaces_around_foreign_characters(text) for text in data if len(text) > 0]
